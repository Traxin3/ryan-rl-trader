                ...
                if expanded not in tested_paths:
                    ...
                    try:
                        result = subprocess.run(
                            [expanded, "--version"], capture_output=True, text=True
                        )
...
def run_backtest():
    ...
    try:
        ...
        results = backtester.run_backtest(
            test_period=test_period, symbol=symbol, timeframe=timeframe
        )
        ...
        print(
            f"📅 Annualized Return:  {colorize_performance(results['annualized_return'])}"
        )
        print(f"⚡ Sharpe Ratio:       {colorize_ratio(results['sharpe_ratio'])}")
        print(f"🛡️  Sortino Ratio:      {colorize_ratio(results['sortino_ratio'])}")
        print(
            f"📉 Max Drawdown:       {colorize_performance(-results['max_drawdown'], -0.05, -0.20)}"
        )
        print(
            f"🎯 Win Rate:           {colorize_performance(results['win_rate'], 0.6, 0.4)}"
        )
        print(
            f"💎 Profit Factor:      {colorize_ratio(results['profit_factor'], 1.5, 1.0)}"
        )
        ...
        if results["total_trades"] > 0:
            avg_trade_return = (
                results["total_return"] / results["total_trades"]
                if results["total_trades"] > 0
                else 0
            )
            print(
                f"📊 Avg Trade Return:   {colorize_performance(avg_trade_return, 0.01, -0.01)}"
            )
        ...
        print(
            f"📊 Performance Chart:  {os.path.abspath('backtest_results/performance.html')}"
        )
        if results["total_trades"] > 0:
            print(
                f"🔍 Trade Analysis:     {os.path.abspath('backtest_results/trade_analysis.html')}"
            )
            print(
                f"📋 Trade Data:        {os.path.abspath('backtest_results/trades.csv')}"
            )
        print("=" * 60)
...
def run_training(config):
    ...
    try:
        ...
        base_cache = (
            f"feature_cache_{'_'.join(symbols)}_{'_'.join(map(str, timeframes))}.pkl"
        )
        try:
            import glob
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gui", action="store_true", help="Start GUI mode with Next.js dashboard"
    )
    ...
    try:
        if args.gui:
            ...
            try:
                ...
                print(
                    "💡 Make sure backend_server.py exists and dependencies are installed"
                )
        elif args.backtest:
            run_backtest()
        elif args.train:
