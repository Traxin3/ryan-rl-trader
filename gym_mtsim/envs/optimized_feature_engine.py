...
class OptimizedFeatureEngine:
    ...
    def load_cache(self):
        ...
        if features is None:
            raise RuntimeError(
                f"Cached features file {cache_file} is missing 'features' key."
            )
        return features
    ...
    def _determine_optimal_components(self, X: np.ndarray) -> int:
        ...
        optimal_components = max(
            self.min_components, min(optimal_components, self.max_components)
        )
    ...
    def fit_transform(self, X: np.ndarray, y: np.ndarray = None) -> np.ndarray:
        ...
        if self.reuse_existing:
            ...
            if features is not None:
                ...
                print(
                    f"✅ Loaded cached features: {features.shape} from {os.path.basename(path)}"
                )
                return features
        ...
        if os.path.exists(cache_file):
            try:
                ...
                if (
                    ...
                    and hasattr(features, "shape")
                    and features.shape[0] == X.shape[0]
                    and obj.get("target_variance", 0) >= self.target_variance * 0.95
                ):
                    self.var_thresh = obj["var_thresh"]
                    self.scaler = obj["scaler"]
                    self.feature_selector = obj.get("feature_selector")
        ...
        print(
            f"📊 PCA: {self.n_components_used} components, {explained_var:.3f} variance explained"
        )
    ...
    def _add_regime_features(
        self, X_pca: np.ndarray, X_original: np.ndarray
    ) -> np.ndarray:
        ...
        if window > 1:
            ...
            vol_regime = (vol_regime - np.mean(vol_regime)) / (
                np.std(vol_regime) + 1e-8
            )
        else:
            vol_regime = np.zeros(n_samples)
