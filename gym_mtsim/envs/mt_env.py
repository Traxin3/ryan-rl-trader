...
class MtEnv(gym.Env):
    ...
    def __init__(
        ...
        self.time_points = (
            self.simulator.symbols_data[(symbols[0], timeframes[0])]
            .index.to_pydatetime()
            .tolist()
        )
        ...
        if self.use_cached_features:
            try:
                ...
                print(
                    f"✅ Loaded cached features with shape {self.signal_features.shape}"
                )
            ...
            if (
                not hasattr(self, "liquidity_features")
                or self.liquidity_features is None
            ):
                self.liquidity_features = self._compute_liquidity_only()
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "liquidity": spaces.Box(
                    low=-INF, high=INF, shape=self.liquidity_shape, dtype=np.float64
                ),
                "orders": spaces.Box(
                    low=-INF,
                    high=INF,
                    dtype=np.float64,
                    shape=(
                        len(self.symbols) * self.symbol_max_orders * 3,
                    ),  # Flatten to 1D to avoid vision network
                ),
            }
        )
        ...
        if len(self.time_points) <= window_size:
            raise ValueError(
                f"Not enough data. Need {window_size+1} bars, got {len(self.time_points)}"
            )
    ...
    def reset(self, seed=None, options=None):
        ...
        if not (
            isinstance(obs, dict)
            and all(isinstance(v, np.ndarray) for v in obs.values())
        ):
            print(
                "[ERROR] MtEnv.reset: Observation is not a dict of np.ndarray!",
                type(obs),
                {k: type(v) for k, v in obs.items()},
            )
        return obs, info
    ...
    def step(self, action: np.ndarray):
        ...
        if len(self.portfolio_values) > 2:
            ...
            self.returns_volatility = (
                float(np.std(returns)) if returns.size > 1 else 0.0
            )
        ...
        if not (
            ...
            and all(isinstance(v, np.ndarray) for v in observation.values())
        ):
            print(
                "[ERROR] MtEnv.step: Observation is not a dict of np.ndarray!",
                type(observation),
                {k: type(v) for k, v in observation.items()},
            )
    ...
    def _calculate_reward(self, orders_info: Dict, closed_orders_info: Dict) -> float:
        ...
        equity_change = (current_equity - prev_info["equity"]) / (
            prev_info["equity"] + 1e-6
        )

        if len(self.history) >= 20:
            recent_equity = [h["equity"] for h in self.history[-20:]]
            equity_returns = np.diff(recent_equity) / (
                np.array(recent_equity[:-1]) + 1e-6
            )
        ...
        exec_entries = [
            e
            for e in getattr(self.simulator, "execution_history", [])
            if e.get("time") == prev_info.get("current_time")
        ]
        is_penalty = 0.0
        adverse_penalty = 0.0
        ...
        for e in exec_entries:
            ...
            total_cost_bps += abs(float(e.get("slippage_bps", 0.0))) + float(
                e.get("spread_bps", 0.0)
            )
        ...
        execution_penalty = -self.execution_cost_penalty_weight * (
        ...
        self.episode_reward_components["participation_penalty"].append(
            ...
            for order in closed_orders:
                ...
                if trade_duration > 0:
                    try:
                        ...
                        exponent = np.log1p(safe_roi) * (
                            24 * 365 / max(trade_duration, 1e-6)
                        )
                ...
                if roi > 0.01 and "holding_time" in order:
                    efficiency_bonus = self.early_close_bonus * min(
                        1.0, roi / (order["holding_time"] + 1e-6)
                    )
        ...
        if len(self.portfolio_values) > 30:
            rets = np.diff(self.portfolio_values) / (
                np.array(self.portfolio_values[:-1]) + 1e-6
            )
            var_p = np.percentile(rets, 5)
            cvar = np.mean(rets[rets <= var_p]) if np.any(rets <= var_p) else 0.0
            tail_penalty = -abs(cvar) * 10.0
        total_risk_penalty = (
            risk_penalty + self.drawdown_penalty * drawdown_penalty + tail_penalty
        )
        ...
        diversification = (
            self.diversification_bonus * unique_symbols / len(self.symbols)
        )

        if len(self.history) >= 10:
            recent_prices = [
                h.get("close_price", h.get("equity", 0)) for h in self.history[-10:]
            ]
            market_volatility = np.std(np.diff(recent_prices)) / (
                np.mean(recent_prices) + 1e-6
            )
    ...
    def _update_position_times(self):
        ...
        for order_id in list(self.position_start_times.keys()):
            if order_id not in active_ids:
                holding_time = (
                    current_time - self.position_start_times.pop(order_id)
    ...
    def _calculate_profit_factor(self) -> float:
        if not self.trade_history:
            return 0.0
        gross_profit = sum(
            trade["profit"] for trade in self.trade_history if trade["profit"] > 0
        )
        gross_loss = abs(
            sum(trade["profit"] for trade in self.trade_history if trade["profit"] < 0)
        )
        ...
        for symbol in self.symbols:
            for tf in self.timeframes:
                ...
                for col in ["Open", "High", "Low", "Close"]:
                    arr = (
                        df[col]
                        .reindex(self.time_points, method="ffill")
                        .values.reshape(-1, 1)
                    )
                    ohlc.append(arr)
    ...
    def _process_data(self) -> np.ndarray:
        ...
        data = self.prices
        dt_index = (
            self.simulator.symbols_data[(self.symbols[0], self.timeframes[0])]
            .reindex(self.time_points, method="ffill")
            .index
        )
        ...
        for idx, dt in enumerate(tqdm(dt_index, desc="Enhanced Liquidity Features")):
            ...
            for symbol in self.symbols:
                for tf in self.timeframes:
                    ...
                    if current_idx >= 10:
                        recent_data = df.iloc[
                            max(0, current_idx - 10) : current_idx + 1
                        ]
                    else:
                        recent_data = df.iloc[: current_idx + 1]
                    recent_prices = recent_data["Close"].values
                    current_price = (
                        float(recent_prices[-1])
                        if len(recent_prices)
                        else float(df.iloc[current_idx]["Close"])
                    )
                    ...
                    for c in ["TickVolume", "Tick_Volume", "Volume", "Real_Volume"]:
                        if c in df.columns:
                            ...
                            break
                    volume_proxy = (
                        float(df.loc[nearest][vol_col])
                        if vol_col
                        else max(1.0, total_depth)
                    )

                    effective_spread_bps = (
                        2.0 * abs(current_price - mid) / max(mid, 1e-9) * 10000.0
                    )
                    ...
                    ofi_proxy = (
                        (delta_mid / max(prev_mid if prev_mid else mid, 1e-9))
                        * total_depth
                        if prev_mid
                        else 0.0
                    )
                    ...
                    dist_liq, liq_sweep, liq_buildup, ob_strength = (
                        sim.get_liquidity_features(symbol, tf, dt)
                    )
                    sessions, mins_since_open, magic1, magic2 = sim.get_session_info(dt)
                    hourly_liquidity = (
                        sim.market_impact_model.get_daily_liquidity_pattern(dt.hour)
                        if hasattr(sim, "market_impact_model")
                        and sim.market_impact_model
                        else 1.0
                    )
        ...
        for idx, dt in enumerate(tqdm(dt_index, desc="Enhanced Market Structure")):
            ...
            for symbol in self.symbols:
                for tf in self.timeframes:
                    ...
                    weak_high, strong_high, weak_low, strong_low = (
                        sim.get_weak_strong_high_low(symbol, tf)
                    )
                    ...
                    current_idx = df.index.get_loc(nearest)
                    if current_idx >= 20:
                        recent_closes = df.iloc[
                            max(0, current_idx - 20) : current_idx + 1
                        ]["Close"].values
                        trend_strength = abs(
                            np.corrcoef(np.arange(len(recent_closes)), recent_closes)[
                                0, 1
                            ]
                        )
            ...
            for symbol in self.symbols:
                for tf in self.timeframes:
                    rr_pot, stop_hunt, atr15, london_bias, ny_bias = (
                        sim.get_trade_risk_features(symbol, tf, dt)
                    )
                    df = sim.symbols_data[(symbol, tf)]
                    nearest = sim.nearest_time(symbol, dt, tf)
                    current_idx = df.index.get_loc(nearest)
                    current_price = df.iloc[current_idx]["Close"]
                    if current_idx >= 24:
                        recent_prices = df.iloc[
                            max(0, current_idx - 24) : current_idx + 1
                        ]["Close"].values
        ...
        if len(core_features) > 1:
            ...
            for i in range(len(close_prices) - 5):
                future_returns[i] = (
                    close_prices[i + 5] - close_prices[i]
                ) / close_prices[i]
            target = np.digitize(
                future_returns, bins=np.percentile(future_returns, [25, 75])
            )

        features_reduced = self.optimized_feature_engine.fit_transform(
            core_features, target
        )
        if (
            self.use_cached_features
            and features_reduced is not None
            and not isinstance(features_reduced, dict)
        ):
        ...
        print(
            f"   - Explained variance: {importance_info.get('total_explained_variance', 'N/A'):.3f}"
        )
        ...
        for i, symbol in enumerate(self.symbols):
            ...
            for j, order in enumerate(symbol_orders):
                if j < self.symbol_max_orders:  # Ensure we don't exceed the limit
                    orders[flat_index : flat_index + 3] = [
                        order.entry_price,
                        order.volume,
                        order.profit,
                    ]
    ...
    def _create_info(self, **kwargs: Any) -> Dict[str, Any]:
        info = {
            ...
            "profit_factor": self._calculate_profit_factor(),
            "avg_trade_profit": (
                np.mean([t["profit"] for t in self.trade_history])
                if self.trade_history
                else 0
            ),
            "avg_holding_time": (
                np.mean([t.get("holding_time", 0) for t in self.trade_history])
                if self.trade_history
                else 0
            ),
            "volatility": self.returns_volatility,
    ...
    def _apply_action(self, action: np.ndarray):
        ...
        for i, symbol in enumerate(self.symbols):
            ...
            order_kind_logit = (
                symbol_action[self.symbol_max_orders] * 5
            )  # market vs limit
            ...
            orders_capacity = self.symbol_max_orders - (
                len(symbol_orders) - len(orders_to_close)
            )
            orders_info[symbol] = {
                "order_id": None,
                "symbol": symbol,
            ...
            if not hold and orders_capacity > 0:
                ...
                try:
                    ...
                    projected_leverage = projected_margin / (
                        self.simulator.balance + 1e-6
                    )
                except Exception:
    ...
    def _get_modified_volume(self, symbol: str, volume_signal: float) -> float:
        ...
        try:
            ...
            margin_cap = max(
                0.0,
                self.max_leverage * (self.simulator.balance + 1e-6)
                - self.simulator.margin,
            )
    ...
    def _estimate_margin(self, symbol: str, volume: float) -> float:
        si = self.simulator.symbols_info[symbol]
        price = float(
            self.simulator.price_at(symbol, self.simulator.current_time)["Close"]
        )
    ...
    def _render_advanced_figure(
        self, figsize=(1400, 600), time_format="%Y-%m-%d %H:%m", return_figure=False
    ):
        fig = go.Figure()
        cmap_colors = np.array(plt_cm.tab10.colors)[[0, 1, 4, 5, 6, 8]]
        cmap = plt_colors.LinearSegmentedColormap.from_list("mtsim", cmap_colors)
        symbol_colors = cmap(np.linspace(0, 1, len(self.symbols)))
        get_color_string = (
            lambda color: f"rgba({color[0]*255}, {color[1]*255}, {color[2]*255}, {color[3]})"
        )
        ...
        for j, symbol in enumerate(self.symbols):
            ...
            for i in range(1, len(self.history)):
                ...
                if order and not order["hold"]:
                    size = 8 + 22 * (
                        order["modified_volume"] / (trade_max_volume + 1e-6)
                    )
                    info = (
