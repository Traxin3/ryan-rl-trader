...
class MarketImpactModel:
    ...
    def _init_or_update_lob(self, symbol: str, current_time: datetime, price_data: np.ndarray, current_price: float) -> None:
        session = self.get_trading_session(current_time)
        liquidity_multiplier = {
            'london': 1.3, 'ny': 1.2, 'overlap': 1.5, 'asian': 0.9, 'off_hours': 0.6
        }.get(session, 1.0)
        ...
        level_sizes = np.array([base_depth * (self.depth_decay ** i) for i in range(self.book_levels)], dtype=float)
        
        level_prices_ask = np.array([ask * (1.0 + (i) * self.level_spacing_bps / 10000.0) for i in range(self.book_levels)], dtype=float)
        level_prices_bid = np.array([bid * (1.0 - (i) * self.level_spacing_bps / 10000.0) for i in range(self.book_levels)], dtype=float)
    ...
    def get_lob_metrics(self, symbol: str, current_time: datetime, price_data: np.ndarray, current_price: float) -> Dict[str, float]:
    ...
    def calculate_slippage(self,
        ...
        """
        volume_impact = self.liquidity_impact * np.log1p(max(volume, 0.0)) / max(liquidity_score, 1e-6)
        ...
        nonlinear_bps = self.impact_coefficient_bps * (max(participation_rate, 0.0) ** self.impact_exponent)
        nonlinear_impact = (nonlinear_bps / 10000.0) * current_price
        
        adverse_bps = max(0.0, trend_sign * (1.0 if order_type == OrderType.Buy else -1.0)) * self.adverse_selection_bps
    ...
    def _estimate_market_notional(self, liquidity_score: float, vol_value: float) -> float:
    ...
    def _walk_book(self, side: OrderType, volume: float, symbol: str) -> Tuple[float, float, float]:
        """Walk the synthetic book to compute VWAP and fills. Returns (vwap, filled, remaining)."""
        book = self._lob[symbol]
        prices: np.ndarray = book['ask_prices'] if side == OrderType.Buy else book['bid_prices']
        sizes: np.ndarray = book['ask_sizes'] if side == OrderType.Buy else book['bid_sizes']
    ...
    def get_execution_price(self,
        ...
        """
        self._init_or_update_lob(symbol, current_time, price_data, current_price)
        book_metrics = self.get_lob_metrics(symbol, current_time, price_data, current_price)
        bid, ask = self.get_dynamic_spread(symbol, current_time, price_data, current_price)
        mid = (bid + ask) / 2.0
        ...
        participation_rate = float(min(1.0, max(0.0, volume / max(est_market_notional, 1e-6))))
        ...
        if market_order:
            ...
            crosses = (limit_price >= ask) if order_type == OrderType.Buy else (limit_price <= bid)
            inside_spread = False
            ...
            if crosses:
                ...
                if order_type == OrderType.Buy:
                    if limit_price >= bid:
                        inside_spread = True
                        queue_level_depth = 0.0
                        inside_spread_distance_ticks = max(0.0, (limit_price - bid) / max(self.tick_size, 1e-12))
                    else:
                        bids = book['bid_prices']
                        idx = int(np.searchsorted(-bids, -limit_price, side='left'))  # bids descending
                    ...
                    if limit_price <= ask:
                        inside_spread = True
                        queue_level_depth = 0.0
                        inside_spread_distance_ticks = max(0.0, (ask - limit_price) / max(self.tick_size, 1e-12))
                    else:
                        asks = book['ask_prices']
                        idx = int(np.searchsorted(asks, limit_price, side='left'))  # asks ascending
        ...
        slippage = self.calculate_slippage(
            order_type, volume=filled, current_price=mid, volatility=vol_value,
            liquidity_score=liquidity_score, participation_rate=participation_rate,
            trend_sign=trend_sign
        ) if filled > 0 else 0.0
        
        effective_spread_bps = float(2.0 * abs(vwap_exec - mid) / max(mid, 1e-9) * 10000.0)
        
        execution_price = float(vwap_exec + (slippage if order_type == OrderType.Buy else -slippage)) if filled > 0 else float(current_price)
        
        base_anchor = (ask if order_type == OrderType.Buy else bid)
        tick_delta = 0.0
        if not market_order:
            tick_delta = (self._quantize_price(limit_unquant) - base_anchor) / max(self.tick_size, 1e-12)
        ...
        execution_info = {
            ...
            'adverse_selection_bps': float(max(0.0, trend_sign * (1.0 if order_type == OrderType.Buy else -1.0)) * self.adverse_selection_bps),
            'session': session,
            'volatility_regime': vol_regime_label,
            'liquidity_score': liquidity_score,
            'total_cost_bps': float(((execution_price - current_price) / max(current_price, 1e-9)) * 10000.0 * (1.0 if order_type == OrderType.Buy else -1.0)),
            'book_total_depth': book_metrics['total_depth'],
            'book_imbalance': book_metrics['imbalance'],
            'tif': str(tif).upper(),
            'limit_offset_bps': float(limit_offset_bps or 0.0) if not market_order else 0.0,
            ...
            'inside_spread_distance_ticks': float(inside_spread_distance_ticks) if not market_order else 0.0,
            'queue_level_depth': float(queue_level_depth) if not market_order else 0.0,
            'queued_volume': float(max(0.0, volume - filled)) if not market_order else 0.0,
            'queue_outflow_rate': float(queue_outflow) if (not market_order and 'queue_outflow' in locals()) else 0.0,
