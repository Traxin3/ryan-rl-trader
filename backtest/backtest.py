...
class VectorBTBacktester:
    ...
    def run_backtest(
        ...
        with tqdm(
            ...
            try:
                while not (done or truncated):
                    ...
                    if (
                        current_step % 50 == 0
                    ):  # Update every 50 steps for smoother display
                        pbar.set_postfix(
                            {
        ...
        final_drawdown = (
            ((max_equity - final_equity) / max_equity) * 100 if max_equity > 0 else 0
        )
    ...
    def _prepare_test_env(
        ...
        if symbol or timeframe:
            ...
            if (
                current_symbols != test_env.symbols
                or current_timeframes != test_env.timeframes
            ):
                print(
                    f"📊 Updating environment for symbol: {current_symbols}, timeframe: {current_timeframes}"
                )
            ...
            try:
                if isinstance(trade, dict):
                    ...
                    if hasattr(order_type, "name"):
                        direction = (
                            "Long"
                            if order_type.name.lower() in ["buy", "long"]
                            else "Short"
                        )
                    elif isinstance(order_type, str):
                        direction = (
                            "Long" if order_type.lower() in ["buy", "long"] else "Short"
                        )
                    elif isinstance(order_type, int):
                        direction = (
                            "Long" if order_type == 0 else "Short"
                        )  # Assuming 0=Buy, 1=Sell
                    else:
                    ...
                    trades.append(
                        {
                            ...
                            / (trade.get("margin", 1.0) + 1e-6),
                            "Direction": direction,
                            "Symbol": trade.get(
                                "symbol", env.symbols[0] if env.symbols else "UNKNOWN"
                            ),
                        }
                    )
            except Exception as e:
    ...
    def _create_vectorbt_portfolio(
        self, price: pd.Series, trades: List[Dict]
    ) -> vbt.Portfolio:
        ...
        if not trades:
            ...
            return vbt.Portfolio.from_holding(
                ...
                fees=(
                    self.env.fee
                    if hasattr(self.env, "fee") and isinstance(self.env.fee, float)
                    else 0.0005
                ),
                freq="1min",
            )
        ...
        print(
            f"📊 Trade time range: {trades_df['EntryTime'].min()} to {trades_df['ExitTime'].max()}"
        )
        ...
        for _, trade in trades_df.iterrows():
            try:
                ...
                try:
                    entry_idx = price.index.get_indexer([entry_time], method="nearest")[
                        0
                    ]
        ...
        print(
            f"✅ Mapped {mapped_entries} entry signals and {mapped_exits} exit signals"
        )
    ...
    def _calculate_metrics(self, pf: vbt.Portfolio, trades: List[Dict]) -> Dict:
        ...
        metrics = {
            ...
            "avg_trade_duration": (
                (trades_df["ExitTime"] - trades_df["EntryTime"]).mean()
    ...
    def _generate_visualizations(
        self, pf: vbt.Portfolio, price: pd.Series, trades: List[Dict]
    ):
        """Generate interactive visualizations of backtest results"""
        try:
            print(
                f"📊 Creating visualizations for portfolio with {pf.orders.count()} orders..."
            )
            ...
            if trades and pf.orders.count() > 0:
                ...
                print(
                    "⚠️ No trades to analyze - portfolio may not have executed properly"
                )
    ...
    def _create_manual_trade_plot(self, price: pd.Series, trades_df: pd.DataFrame):
        ...
        try:
            ...
            for _, trade in trades_df.iterrows():
                try:
                    ...
                    if entry_time in price.index:
                        ...
                        entry_idx = price.index.get_indexer(
                    ...
                    if exit_time in price.index:
                        ...
                        exit_idx = price.index.get_indexer(
                            [exit_time], method="nearest"
                        )[0]
                        exit_price = (
                            price.iloc[exit_idx]
                            if exit_idx >= 0
                            else trade["ExitPrice"]
                        )
            ...
            print(
                "✅ Manual trade analysis saved to backtest_results/manual_trade_analysis.html"
            )
