...
def simulate_trading_metrics():
    ...
    time_elapsed = (
        time.time() - training_state["start_time"]
    ...
    if training_state["is_training"]:
        ...
        current_drawdown = (
            training_state["peak_equity"] - training_state["equity"]
        ) / training_state["peak_equity"]
        training_state["max_drawdown"] = max(
            training_state["max_drawdown"], current_drawdown
        )

        training_state["win_rate"] = min(
            0.95, 0.5 + (training_state["steps"] / 1500000) * 0.3
        )
        training_state["sharpe_ratio"] = min(
            3.0, 0.5 + (training_state["steps"] / 1500000) * 2.0
        )
        training_state["profit_factor"] = min(
            3.0, 1.0 + (training_state["steps"] / 1500000) * 1.5
        )
        training_state["sortino_ratio"] = min(
            4.0, 0.5 + (training_state["steps"] / 1500000) * 2.5
        )
        training_state["volatility"] = max(
            0.05, 0.2 - (training_state["steps"] / 1500000) * 0.1
        )

        if np.random.random() < 0.1:  # 10% chance of new trade
            training_state["total_trades"] += 1
            training_state["avg_trade_profit"] = (
                training_state["avg_trade_profit"]
                * (training_state["total_trades"] - 1)
                + np.random.normal(20, 50)
            ) / training_state["total_trades"]

        training_state["avg_holding_time"] = max(
            1.0, 5.0 - (training_state["steps"] / 1500000) * 3.0
        )
    ...
    return jsonify(
        {
            ...
            "history": (
                metrics_history[-100:] if metrics_history else []
            ),  # Last 100 points
        }
    )
...
def get_config():
    ...
    config = load_config()
    if config:
        return jsonify({"env": config.get("env", {})})
